...
export class Chainable {
	...
	*
	* @param {*} [newState] The possible new state to replace, if falsy `$self` is returned instead
	*
	* @returns {Chainable} This chainable wrapper
	*/
	$replace(newState) {
		if (typeof newState == 'object') this.$source = newState;
		return this.$self;
	}
	...
	*
	* @param {String} method The source method to call - can be in dotted or array notation for nested methods
	* @param {*...} [args] Additional arguments to pass to the method
	*
	* @returns {Chainable} This chainable wrapper
	*/
	$call(method, ...args) {
		let [context, func] = (Array.isArray(method) ? method : method.split('.'))
			.reduce((iter, segment) =>
				!iter ? undefined : [iter[1], iter[1][segment]]
			, [this.$source, this.$source]);

		return this.$replace(func.call(context, ...args));
	}


	/**
	* Get a (possibly) nested value from the source object
	* This is mainly used internally to access deep properties
	*
	* @param {String} path The method to retrieve, can be in dotted or array notation
	* @returns {*} The source value at the given path
	*/
	$get(path) {
		return (Array.isArray(path) ? path : path.split('.'))
			.reduce((iter, segment) =>
				!iter ? undefined : iter[segment]
			, this.$source);
	}
	...
	*/
	$set(key, val) {
		if (typeof key == 'string' || Array.isArray(key)) { // Simple or nested key setter
